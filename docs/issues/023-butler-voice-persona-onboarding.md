...
- Butler が応答を音声出力できる。
- 音声出力は device / browser の Speech Synthesis を第一候補にする。
- voice preset は「落ち着いた年配の仕事のできる執事長」を目標にし、端末で選べる日本語 voice、rate、pitch で可能な範囲に寄せる。
- 端末 voice で理想声が作れない場合は、外部 TTS / Cloudflare TTS / Kokoro を使わず text persona に degrade できる。
...
- 聞き取った呼称の即時自動確定。
- Cloudflare Workers AI / Kokoro / VOICEVOX / COEIROINK を default 音声にすること。
